...
from har_analyzer.utils.exceptions import HARParsingError, InvalidHARFileError, ValidationError
...
class TestHARParser:
    ...
    def test_parse_valid_har_file(self, sample_har_file: Path):
        ...
        assert list(df.columns) == [
            'url', 'method', 'status_code', 'type', 'mime_type',
            'response_time_ms', 'size_bytes', 'size_kb', 'start_time',
            'timing_blocked', 'timing_dns', 'timing_connect', 
            'timing_send', 'timing_wait', 'timing_receive'
        ]
